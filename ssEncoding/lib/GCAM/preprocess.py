...
def draw_shape_by_detail(dfg,df_detail,oid,isLegend=False,cols=['xs','ys']):
    ...
    for i in range(points):
        if dft[dft['PID']==i].__len__()==0:
            continue
        plt.scatter(dft[dft['PID']==i][cols[0]].values,dft[dft['PID']==i][cols[1]].values,label=dft[dft['PID']==i]['PID'].values[0])
...
def get_shape_mbr(df_shape):
    ...
    #
    df_mbr['l1']=pd.Series([cal_euclidean([x0,y0],[x1,y1])for x0,y0,x1,y1 in df_mbr[['x0','y0','x1','y1']].values])
    df_mbr['l2']=pd.Series([cal_euclidean([x0,y0],[x1,y1])for x0,y0,x1,y1 in df_mbr[['x1','y1','x2','y2']].values])

    df_mbr['a1']=pd.Series([cal_arc([x0,y0],[x1,y1],True)for x0,y0,x1,y1 in df_mbr[['x0','y0','x1','y1']].values])
    df_mbr['a2']=pd.Series([cal_arc([x0,y0],[x1,y1],True)for x0,y0,x1,y1 in df_mbr[['x1','y1','x2','y2']].values])
    #
...
def get_shape_rotate(df_shape):
    ...
    df_aff['geometry']=pd.Series([affinity.rotate(geo,lon_arc) for geo,lon_arc in df_aff[['geometry','lon_arc']].values])
...
def get_shape_features(gdf):
    ...
    outlist=[x for x in ['OID','OBJECTID','Floor','geometry','length','area','xy','p_num','o_x','o_y','lon_len','short_len'] if x in gdf.columns]
    gdf=gdf[outlist]
...
def simplify_cos_on_node(df_node,tor_cos):
    ...
    df_line['x_l']=pd.Series([coor_dic[(oid,(pid-1 if pid>=1 else pnum-2))][0] for oid,pid,pnum in df_line[['OBJECTID','PID','p_num']].values])
    df_line['y_l']=pd.Series([coor_dic[(oid,(pid-1 if pid>=1 else pnum-2))][1] for oid,pid,pnum in df_line[['OBJECTID','PID','p_num']].values])
    df_line['x_r']=pd.Series([coor_dic[(oid,(pid+1 if pid<(pnum-2) else 0))][0] for oid,pid,pnum in df_line[['OBJECTID','PID','p_num']].values])
    df_line['y_r']=pd.Series([coor_dic[(oid,(pid+1 if pid<(pnum-2) else 0))][1] for oid,pid,pnum in df_line[['OBJECTID','PID','p_num']].values])
    #
    ...
    df_line['cos']=pd.Series([(dxl*dxr+dyl*dyr)/(np.sqrt(dxl*dxl+dyl*dyl)*np.sqrt(dxr*dxr+dyr*dyr)) for dxl,dyl,dxr,dyr in df_line[['dx_l','dy_l','dx_r','dy_r']].values])
    #
...
def simplify_dp_on_shape(df_shape,tor=0.000001,type=1):
    ...
    if type==1:
        df_shape['geometry']=pd.Series([geo.simplify(tor*diag) for geo,diag in df_shape[['geometry','diag']].values])
    else:
...
def get_shape_normalize(df_use,if_scale_y):
    ...
    #
    df_node['ddx']=pd.Series([dl*((xr-mu_x)**2+(x-mu_x)**2+(x-mu_x)*(xr-mu_x))/3 for dl,x,xr,mu_x in df_node[['dl','x','xr','mu_x']].values])
    ...
    #
    df_node['ddy']=pd.Series([dl*((xr-mu_x)**2+(x-mu_x)**2+(x-mu_x)*(xr-mu_x))/3 for dl,x,xr,mu_x in df_node[['dl','y','yr','mu_y']].values])
    ...
    df_use['geometry']=pd.Series([affinity.translate(geo,-mx,-my) for mx,my,geo in df_use[['mu_x','mu_y','geometry']].values])
    #
    if if_scale_y:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_y) for del_x,del_y,geo in df_use[['del_x','del_y','geometry']].values])
    else:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_x) for del_x,geo in df_use[['del_x','geometry']].values])
...
def get_shape_maxmin(df_use,if_scale_y):
    ...
    #-1~1
    if if_scale_y:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_y) for del_x,del_y,geo in df_use[['scale_x','scale_y','geometry']].values])
    else:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_x) for del_x,geo in df_use[['scale_x','geometry']].values])
    #
...
def get_line_features(df_node,POINTS_SHAPE=20):
    ...
    #
    df_line['xs']=pd.Series([[x+(i+1)*_dx for i in range(int(dc))] for x,_dx,dc in df_line[['x0','_dx','dc']].values])
    df_line['ys']=pd.Series([[y+(i+1)*_dy for i in range(int(dc))] for y,_dy,dc in df_line[['y0','_dy','dc']].values])
    #
    ...
    df_line.drop(['length', 'p_num','next_x', 'next_y','dl', 'last_cum_dr', 'cum_count', 'last_cum', '_dr', '_dx', '_dy', 'x0', 'y0'],axis=1,inplace=True)
...
def get_single_features(df_detail,k):
    ...
    df_detail['temp']=pd.Series([oid*1000+temp if temp>=0 else oid*1000+uid_max+1+temp for uid_max,temp,oid in df_detail[['uid_max','temp','OBJECTID']].values])
    ...
    #C
    df_detail['temp']=df_detail['UID']+k
    df_detail['temp']=pd.Series([oid*1000+temp if temp<=uid_max else oid*1000+temp-uid_max-1 for uid_max,temp,oid in df_detail[['uid_max','temp','OBJECTID']].values])
    ...
    #
    # AB,AC,BC,OA,OB,OC
    df_detail['l_ab']=pd.Series([cal_euclidean([ax,ay],[bx,by]) for ax,ay,bx,by in df_detail[['a_x','a_y','b_x','b_y']].values])
    df_detail['l_ac']=pd.Series([cal_euclidean([ax,ay],[cx,cy]) for ax,ay,cx,cy in df_detail[['a_x','a_y','c_x','c_y']].values])
    df_detail['l_bc']=pd.Series([cal_euclidean([cx,cy],[bx,by]) for cx,cy,bx,by in df_detail[['c_x','c_y','b_x','b_y']].values])
    df_detail['l_oa']=pd.Series([cal_euclidean([ax,ay],[ox,oy]) for ax,ay,ox,oy in df_detail[['a_x','a_y','o_x','o_y']].values])
    df_detail['l_ob']=pd.Series([cal_euclidean([bx,by],[ox,oy]) for bx,by,ox,oy in df_detail[['b_x','b_y','o_x','o_y']].values])
    df_detail['l_oc']=pd.Series([cal_euclidean([cx,cy],[ox,oy]) for cx,cy,ox,oy in df_detail[['c_x','c_y','o_x','o_y']].values])
    #
    # 

    df_detail['arc_ba']=round(1-np.arctan2(df_detail['a_y']-df_detail['b_y'],df_detail['a_x']-df_detail['b_x'])/np.pi,6)
    df_detail['arc_ac']=round(1-np.arctan2(df_detail['c_y']-df_detail['a_y'],df_detail['c_x']-df_detail['a_x'])/np.pi,6)
    df_detail['arc_ob']=round(1-np.arctan2(df_detail['b_y']-df_detail['o_y'],df_detail['b_x']-df_detail['o_x'])/np.pi,6)
    df_detail['arc_oc']=round(1-np.arctan2(df_detail['c_y']-df_detail['o_y'],df_detail['c_x']-df_detail['o_x'])/np.pi,6)
    # 
    ...
    #
    #Area of Tri_ABC
    df_detail['s_abc']=pd.Series([(-1 if angle<1 else 1)*cal_area(l1,l2,l3) for l1,l2,l3,angle in df_detail[['l_ab','l_bc','l_ac','angle_bac']].values])
    #Area of Tri_OBC
...
def get_normalize_features(df_features,norm_type):
    ...
    cols=[x for x in df_features.columns if 'k' in x and 'rotate' not in x]+['Elongation','Circularity','Rectangularity','Convexity','l_oa','MeanRedius']
...
def get_train_sequence(df_features,cols,rotate_type):
    ...
    if rotate_type=='vertex':
        beginDic={group[0]:list(group[1]['UID'].values) for group in df_features[df_features['isBegin']==1][['OBJECTID','UID']].groupby('OBJECTID')}
    elif rotate_type=='equal':
        beginDic={group[0]:list(group[1]['UID'].values) for group in df_features[df_features['isStart']==1][['OBJECTID','UID']].groupby('OBJECTID')}
    elif rotate_type=='none':
        beginDic={group[0]:list(group[1]['UID'].values) for group in df_features[df_features['UID']==0][['OBJECTID','UID']].groupby('OBJECTID')}
    maxLen=max([len(value) for key,value in beginDic.items()])
    #
    features_list={group[0]:list(group[1]['features'].values) for group in df_features[['OBJECTID','features']].groupby('OBJECTID')}
    #
    features_list_list={key:[value[begin:]+value[:begin] for begin in beginDic[key]] for key,value in features_list.items()}
...
def get_normalize_features(df_features,norm_type):
    df_features=copy.deepcopy(df_features)
    cols=[x for x in df_features.columns if 'k' in x and 'rotate' not in x]+['l_oa','Area','Perimeter','Elongation','Circularity','MeanRedius']
    #
    df_stat=df_features[cols].describe().transpose()
    for col in cols:
        col_min,col_max,col_std,col_mean=df_stat.loc[col][['min','max','std','mean']].values
...
def peeky_AE_model(hparams,features_len):
    ...
    if hparams['GPU']:
        ...
        encoder_lstm=rnn(hparams['z_size'],return_state=True,return_sequences=False,name='encoder_lstm',dropout=hparams['dropout'],recurrent_dropout=hparams['rnn_dropout'])
        decoder_lstm=rnn(hparams['z_size'],return_sequences=True, return_state=False,name='decoder_lstm',dropout=hparams['dropout'],recurrent_dropout=hparams['rnn_dropout'])
    #Encoder,init_h, init_c
    encoder_input=keras.Input(shape=(hparams['seq_length'],features_len),name='encoder_input')
    encoder_output,*encoder_states=encoder_lstm(encoder_input)
...
def get_shape_normalize_final(df_use, if_scale_y):
    ...
    #-1~1
    if if_scale_y:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_y,origin='centroid') for del_x,del_y,geo in df_use[['scale_x','scale_y','geometry']].values])
    else:
        df_use['geometry']=pd.Series([affinity.scale(geo,1/del_x,1/del_x,origin='centroid') for del_x,geo in df_use[['scale_x','geometry']].values])
    df_use.drop(['mu_x','mu_y','scale_x','scale_y','x_max','x_min','y_max','y_min'],axis=1,inplace=True)
...
def get_overall_features_final(df_features,df_use):
    ...
    dft['Elongation']=dft['lon_len']/dft['short_len']
    dft['Circularity']=4*np.pi*dft['Area']/dft['Perimeter']/dft['Perimeter']
    dft['Rectangularity']=pd.Series([area/geo.minimum_rotated_rectangle.area for area,geo in dft[['Area','geometry']].values])
...
def get_single_features_final(df_detail,k):
    ...
    df_detail['angle_bac_change']=pd.Series([change if change<=1 else change-2 for change in df_detail['angle_bac_change']])
...
#     #
    df_detail['s_abc']=pd.Series([(-1 if angle<1 else 1)*feat/area for feat,area,angle in df_detail[['s_abc','Area','angle_bac']].values])
    df_detail['l_bc']= pd.Series([(-1 if angle<1 else 1)*feat/np.sqrt(area) for feat,area,angle in df_detail[['l_bc','Area','angle_bac']].values])
