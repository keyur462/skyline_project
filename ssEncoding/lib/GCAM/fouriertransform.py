...
def fourier_Series(X,S,series_size):
	...
	for i in range(0,series_size):
		...
		for j in range(0, coord_size - 1):
			...
			# define the convolutional function of X(s) to obtain XA,XB.
			fax = lambda s: (X[j] + (X[j+1] - X[j]) * (s - s0) / (s1 - s0)) * math.cos(i * 2 * math.pi * s / period_size) * 2 / period_size
			fbx = lambda s: (X[j] + (X[j+1] - X[j]) * (s - s0) / (s1 - s0)) * math.sin(i * 2 * math.pi * s / period_size) * 2 / period_size

			try:
				XAi += integrate.quad(fax, s0, s1, limit=100)[0]
				XBi += integrate.quad(fbx, s0, s1, limit=100)[0]
			except Exception as e:
				print(f"[Warning] Integration failed at segment {j}: {e}")
				continue
...
def fouriertransform(A, series_size, is_adjusted=True, is_normalized=False):
	X, Y, S = convertGeoObjectToArcLengthFunction(A, is_adjusted=is_adjusted, is_normalized=is_normalized)
	coord_size = len(X)
	if(coord_size == 0):
		print('ILLEGAL_ARGUMENT')
	...
	XA, XB = fourier_Series(X, S, series_size)
	YA, YB = fourier_Series(Y, S, series_size)
	composite_A_vector = XA + XB + YA + YB
	return np.array(composite_A_vector)

def distance_between_two_transforms(transform_1, transform_2):
	dis = 0.
	assert len(transform_1) == len(transform_2)
	for i in range(0, len(transform_1)):
		#dis += pow((transform_1[i]/transform_1[0]-transform_2[i]/transform_2[0]), 2)
		dis += pow((transform_1[i]-transform_2[i]), 2)
	return math.sqrt(dis) # get_distance(composite_A_vector,composite_B_vector,1)
